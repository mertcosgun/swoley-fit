...
export default function ExerciseCard(props) {
  ...
  return (
    <div className="p-4 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-wrap">
      ...
      <div className="flex flex-col bg-slate-950 rounded gap-2s">
        {exercise.description.split("___").map((val) => {
          return (
            <div key={val.length} className="text-sm">
              {val}
            </div>
          );
        })}
      ...
      <div className="grid  grid-col-2 sm:grid-cols-4 sm:place-item-center gap-2">
